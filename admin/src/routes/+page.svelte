<script>
    ...
    async function deleteArticle(id) {
        ...
        try {
            await fetch(`/api/article?id=${id}`, { method: 'DELETE' });
            await loadPosts();
        } catch (err) {
            console.error(err);
...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a class="btn btn-success btn-sm" href="/add-article">+ Add New Post</a>
    </div>
                ...
                <tbody>
                    {#each articles as article}
                        <tr>
                            ...
                            <td>
                                <div class="btn-group">
                                    <a class="btn btn-sm btn-outline-warning" href="/edit-article?id={article.id}">Edit</a>
